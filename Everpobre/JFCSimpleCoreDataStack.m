...
@interface JFCSimpleCoreDataStack ()
...
@implementation JFCSimpleCoreDataStack
    ...
    if (_storeCoordinator == nil) {
        ...
        if (![_storeCoordinator addPersistentStoreWithType:NSSQLiteStoreType
            ...
            NSNotification *note = [NSNotification
                                    notificationWithName:[JFCSimpleCoreDataStack persistentStoreCoordinatorErrorNotificationName]
                                    object:self
                                    userInfo:@{@"error" : err}];
...
+ (JFCSimpleCoreDataStack *)coreDataStackWithModelName:(NSString *)aModelName databaseFilename:(NSString*) aDBName
{
    NSURL *url = nil;
...
+ (JFCSimpleCoreDataStack *)coreDataStackWithModelName:(NSString *)aModelName
{
    return [self coreDataStackWithModelName:aModelName databaseFilename:nil];
}

+ (JFCSimpleCoreDataStack *)coreDataStackWithModelName:(NSString *)aModelName databaseURL:(NSURL *)aDBURL
{
...
- (void)saveWithErrorBlock:(void(^)(NSError *error))errorBlock
{
    ...
    if (!_context) {
        err = [NSError errorWithDomain:@"JGCSimpleCoreDataStack"
                                  code:1
                              userInfo:@{NSLocalizedDescriptionKey :
                                             @"Attempted to save a nil NSManagedObjectContext. This JFCSimpleCoreDataStack has no context - probably there was an earlier error trying to access the CoreData database file."}];
        errorBlock(err);
...
-(NSArray *)executeRequest:(NSFetchRequest *)request
    ...
    if (!_context) {
        err = [NSError errorWithDomain:@"JFCSimpleCoreDataStack"
                                  code:1
                              userInfo:@{NSLocalizedDescriptionKey :
                                             @"Attempted to search a nil NSManagedObjectContext. This JFCSimpleCoreDataStack has no context - probably there was an earlier error trying to access the CoreData database file."}];
        errorBlock(err);
